...
async function submitCartOrder() {
    ...
    try {
        ...
        console.error('submitCartOrder error:', error);
    } finally {
        if (submitBtn) {
    ...
    return productsArray.map(p => {
        ...
        return `
            <div class="product-card" data-id="${p.id}">
                <img class="product-img" src="${p.image || 'https://via.placeholder.com/200x150?text=Alko'}" alt="${escapeHtml(p.name)}" loading="lazy">
